        ...
        <div className="grid grid-cols-1 lg:grid-cols-[380px_1fr] gap-8 lg:gap-12">
          ...
          <div>
            ...
            {isLoading ? (
              ...
              <>
                {/* Desktop Table - only on PC when NOT admin */}
                <div
                  className={`hidden ${isAdmin ? 'md:hidden' : 'md:block'} overflow-x-auto`}
                  style={{
                    background: 'var(--color-surface-card)',
                    border: '1px solid rgba(255,255,255,0.06)',
                    borderRadius: 'var(--border-radius-lg)',
                ...
                {/* Cards - mobile always, PC only when admin */}
                <div className={`space-y-3 ${isAdmin ? '' : 'md:hidden'}`}>
                  {participants.map((p) => (
                    <div
                      key={p.id}
                      className="p-4 md:max-w-[720px] md:mx-auto"
                      style={{
                        background: 'var(--color-surface-card)',
                        border: '1px solid rgba(255,255,255,0.06)',
